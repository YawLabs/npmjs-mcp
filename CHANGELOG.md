...
- package.json keywords: the core discoverability terms (claude-code, cursor, ai-agents, ai) now come before the topic-specific ones, so they survive GitHub's 20-topic cap when topics are synced from keywords.
...
- **An oam host older than the floor no longer serves the server itself.** When a client ran `oam run bin/npmjs-mcp.mjs` with an old oam and discovery came up empty, the server ran on that old oam. An old host now hands off to the newest usable oam, or to Node on `PATH`, or exits with an error when there is neither. Every handoff from an oam host pipes stdio instead of inheriting it: an oam older than 0.9.0 treats `stdio: 'inherit'` as `'pipe'`, so an inherited handoff never completes the MCP handshake (measured on a real oam 0.8.2 host). The pipes, signal forwarding and exit mirror are connected only once the child has actually spawned, so a chosen oam that fails to launch (deleted or replaced after its version check) still falls back instead of taking the launcher down with the failed spawn's exit code.
- **A bad `OAM_BIN` is reported instead of silently ignored.** A path that does not exist, an oam below the floor, or a binary that will not run is always named on stderr, and discovery carries on instead of dropping straight to Node. Discovered binaries that lose to a newer one are not mentioned; they, and an `oam.cmd` / `oam.bat` shim, are named only when no usable oam is found.
- **`NPMJS_MCP_RUNTIME=node` now always means Node.** Launched under `oam run`, it hands off to Node on `PATH` rather than staying on oam. It still never applies `NPMJS_MCP_SANDBOX=1`.
- **A requested sandbox that cannot be applied is now said out loud.** `NPMJS_MCP_SANDBOX=1` still forces a freshly spawned oam, and under `auto` still falls back without `--permission` when no usable oam is found or the one found fails to launch — but that fallback now writes a note to stderr instead of dropping the sandbox silently. On an oam host at the floor the fallback serves in that process; below the floor it hands off to Node. `NPMJS_MCP_RUNTIME=oam` still makes it fatal. The `--permission` / `--allow-net` / `--allow-env` grants were re-verified with a real MCP handshake on oam 0.15.2.
- Each `oam --version` probe is bounded at 5s, so a wedged binary on `PATH` cannot hang the launch.

### Documentation
- The README's Runtime section no longer says oam's `--permission` sandbox is "deliberately not used" — `NPMJS_MCP_SANDBOX=1` has offered it since 0.15.0 — and the configuration table now lists `NPMJS_MCP_SANDBOX` and describes the 0.15.2 floor and newest-oam selection.
...
- The launcher's own header comment claimed a requested sandbox "still spawns" a fresh oam; it now describes the `auto` fallback above instead of overstating the guarantee.
...
- The 0.15.0 entry below was backfilled from its release range and verified against the shipped code; that version had gone out without one.
...
### Added
- **`NPMJS_MCP_SANDBOX=1` runs the server under oam's `--permission` sandbox**, superseding the "deliberately unused" note published in 0.14.0. That note stood because `--permission` denied network with no grant available to open it, so the server completed the MCP handshake and then failed every tool call; oam 0.8.3 added `--allow-net` / `--allow-env` and made it viable. Filesystem and subprocess stay denied outright — no `--allow-fs-read/write`, no `--allow-child-process` — which is the entire point: this server reads no files at runtime (its version is baked in at build time) and spawns nothing, so a dependency that suddenly wants either is stopped by the runtime rather than trusted. Meaningful for a process holding an `NPM_TOKEN`. The grants are oam **process-level** flags and are passed *before* the `run` subcommand; `oam run --permission …` is rejected outright ("unexpected argument '--permission' found"), so the argument ordering is load-bearing.
- **The sandbox is opt-in rather than default because a wrong grant list does not fail loudly.** Measured on oam 0.9.0 with `--allow-net=registry.npmjs.org` alone: `npm_downloads` failed outright, but `npm_health` returned HTTP 200 with `weeklyDownloads` null and *no error at all* — the download counts come from `api.npmjs.org`, a second host. A silently half-populated answer is worse than a refusal, so the shipped grant is derived from what the bundle actually reaches (`registry.npmjs.org`, `api.npmjs.org`, `replicate.npmjs.com`) rather than guessed, plus the parsed hostname of `NPM_REGISTRY` when a private registry is configured. The environment is the same hazard, worse: oam denies a non-granted variable by making it **absent** from `process.env` rather than throwing, so an under-granted `NPM_TOKEN` reads as "unauthenticated" instead of "denied". That list is exactly the set the bundle reads — `NPM_TOKEN`, `NPM_REGISTRY`, `NPM_REQUEST_TIMEOUT_MS`, `NPM_RETRY_BACKOFF_MS`, `DEBUG` — and has to be kept in step with it.
- **The test suite asserts `server.json` and `package.json` stay in sync** (#36), run as part of `npm test`. `server.json` is what the Official MCP Registry reads at publish time. It carries the version twice (top-level plus `packages[].version`) and `release.sh` syncs it separately from `package.json`, so an edit that updates one and not the other ships a desynced registry entry — visible to users, invisible to the release. Three assertions: top-level version parity, per-package version parity, and `package.json` `mcpName` equal to `server.json` `name`. That last one is a different drift mode — the registry keys the package on `name` while npm consumers read `mcpName`, so disagreement puts discovery and install on different identifiers. The `mcpName` check asserts both fields are non-empty first, so it cannot pass vacuously if a refactor drops both. Ported from tailscale-mcp, which was the only server that had the check when this landed. It had already earned its keep there: during tailscale-mcp's own 0.15.0 release a hand-pre-synced `server.json` at 0.15.0 disagreed with a `package.json` still at 0.14.0, and the suite aborted that run at `release.sh` step 2 — the fix being to let `release.sh` own the bump. Nothing was caught in this repo's own release.

### Changed
- **The oam path now requires oam 0.9.0 or newer.** It now probes `oam --version` once before deciding to spawn; below the floor, `auto` falls back to Node with a note on stderr and `NPMJS_MCP_RUNTIME=oam` is a hard error. Older oam ran `child_process.execFile` arguments through a **shell**, accepted `exec`'s `timeout` and ignored it, and truncated `spawnSync` at `maxBuffer` while reporting success. This server spawns nothing, so the floor is enforced for consistency with the rest of `@yawlabs/*-mcp` rather than because this launcher is exposed — which is why an older oam degrades rather than failing. Discovery itself stays stat-only; the probe is the launcher's first subprocess, measured at a 26 ms median (n=12, windows-arm64) against a launch that was going to spawn a process regardless, and paid once per MCP session. A pre-release suffix (`0.9.0-rc.1`) is truncated at the first non-numeric character so it compares as its base version.
